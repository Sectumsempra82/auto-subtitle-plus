...
The v0.3.0-rc.4 Apple Silicon `.app` bundles Python, Qt and the processing libraries. It does not depend on a checkout or virtual environment. FFmpeg/ffprobe must be installed separately; models download on demand. No media, settings, model weights or credentials belong in the bundle.
...
.venv/bin/python tools/package_macos.py --build-number 5
```
...
build 3; build 4 was a local installer prototype. The rc.4 release uses build 5. The number becomes the app's
...
to ZIP for older releases. Static download links point to the rc.4 installer, so they also work when the
