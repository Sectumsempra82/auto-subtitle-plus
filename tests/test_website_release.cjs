...
(async()=>{
  ...
  const installerOnly = await run([{...installer, assets:installer.assets.slice(1)}]);
  assert.match(installerOnly.links[2].href,/\.pkg$/);
  const newerZip = {...mac, tag_name:'newer', published_at:'2026-09-22', assets:[asset('newer', 'AutoSubtitlePlus-GUI-macOS-arm64.zip')]};
  assert.match((await run([installer,newerZip])).links[2].href,/\/newer\//);
  const invalidInstaller = {...mac, assets:[{...installer.assets[1], browser_download_url:'https://invalid.example/app.pkg'}, ...mac.assets]};
  assert.match((await run([invalidInstaller])).links[2].href,/\.zip$/);
  ...
  console.log('Release links: installer preference, ZIP compatibility, platform selection, drafts, URL validation and offline fallback passed.');
})();
